...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
	{
		...
		#define AVERAGE_WINDOW_SIZE 20  // Number of samples to average over

		static uint16_t period_samples[AVERAGE_WINDOW_SIZE];
		...
		period_samples[sample_index] = last_period;
		sample_index = (sample_index + 1) % AVERAGE_WINDOW_SIZE;

		if (sample_count < AVERAGE_WINDOW_SIZE) {
			sample_count++;
		}

		// Compute average only if buffer is filled
		if (sample_count == AVERAGE_WINDOW_SIZE) {
			for (int i = 0; i < AVERAGE_WINDOW_SIZE; i++) {
				sum += period_samples[i];
			}
			uint16_t avg_period = sum / AVERAGE_WINDOW_SIZE;

			// Apply height conversion logic to avg_period
			const uint16_t max_height = 1000; // mm
			const uint16_t min_height = 100;  // mm
			uint8_t claw_height_percent;

			if (avg_period <= min_height) {
				claw_height_percent = 0;
			} else if (avg_period >= max_height) {
				claw_height_percent = 100;
			} else {
				uint16_t usable_range = max_height - min_height;
				claw_height_percent = ((avg_period - min_height) * 100) / usable_range;
			}
